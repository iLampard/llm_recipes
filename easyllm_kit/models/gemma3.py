...
class Gemma3(LLM):
    ...
    def load_model(self):
        ...
        try:
            ...
            self.model = Gemma3ForConditionalGeneration.from_pretrained(
                model_id,
                device_map=self.model_config.device_map or "auto",
    ...
    def generate(self, prompts: Union[str, List[str]], **kwargs) -> Union[str, List[str]]:
        """
        Generate text based on the input prompt(s).

        Args:
            prompts (Union[str, List[str]]): The input prompt(s) for text generation.
            **kwargs: Additional arguments for generation, including:
                - system_message: Optional system message to guide the model
                - image_path: Optional path to an image for multimodal generation

        Returns:
            Union[str, List[str]]: The generated text.
        """
        ...
        for prompt in prompts:
            try:
                # Prepare messages
                messages = format_prompt_with_image(prompt, image_dir)
